...
class RolloutWorker:
    ...
    def generate_episode(self, episode_num=None, evaluate = False):
        ...
        # }
        wrap_obs = self.env.get_obs()
        obs = wrap_obs['Box']
        state = wrap_obs['State']
        while not terminated and step < self.episode_limit:
            ...
            new_obs_n, reward, terminated, info = self.env.step({'Discrete': actions})
            wrap_obs = new_obs_n
            obs = wrap_obs['Box']
            state = wrap_obs['State']
            # reward, terminated, info = self.envs[episode_num].step(actions)
        ...
        if evaluate and episode_num == self.args.evaluate_epoch - 1 and self.args.replay_dir != '':
            ...
            self.env.close()
        retdict['collection_ratio'] = info['a_poi_collect_ratio']
        retdict['violation_ratio'] = info['b_emergency_violation_ratio']
        retdict['episodic_aoi'] = info['e_weighted_aoi']
        retdict['T-AoI'] = info['f_weighted_bar_aoi']
        retdict['consumption_ratio'] = info['h_energy_consuming_ratio']
        print('---------generate end---------')
        return episode, episode_reward, retdict, step
        # return episode_reward, retdict, step
...
class CommRolloutWorker:
    ...
    def generate_episode(self, episode_num=None, evaluate=False):
        ...
        while not terminated and step < self.episode_limit:
            ...
            # weights = [1 for _ in range(self.n_agents)]
